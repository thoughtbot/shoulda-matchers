...
describe Shoulda::Matchers::ActionController::RedirectViaTurbolinksToMatcher do
  context 'a controller that redirects' do
    ...
    it 'rejects redirecting to a different url' do
      expect(controller_redirecting_via_turbolinks_to('/some/url')).
        not_to redirect_via_turbolinks_to('/some/other/url')
    end

    it 'accepts redirecting to that url in a block' do
      expect(controller_redirecting_via_turbolinks_to('/some/url')).
        to redirect_via_turbolinks_to('somewhere') { '/some/url' }
    end

    it 'rejects redirecting to a different url in a block' do
      expect(controller_redirecting_via_turbolinks_to('/some/url')).
        not_to redirect_via_turbolinks_to('somewhere else') { '/some/other/url' }
    end

    def controller_redirecting_via_turbolinks_to(url)
      build_response { redirect_via_turbolinks_to url }
    end
  end
  ...
  context 'a controller that does not redirect' do
    it 'rejects redirecting to a url' do
      ...
      expect(controller).not_to redirect_via_turbolinks_to('/some/url')
    end
  end

  it 'provides the correct description when provided a block' do
    matcher = redirect_via_turbolinks_to('somewhere else') { '/some/other/url' }
