...
describe Shoulda::Matchers::ActiveModel::ComparisonMatcher do
  context 'with a model with a greater than validation' do
    it 'match accordingly' do
      comparison_instance(:greater_than => 2).should matcher.is_greater_than(2)
    end
  end

  context 'with a model with a greater than or equal to validation' do
    it 'match accordingly' do
      comparison_instance(:greater_than_or_equal_to => 2).should matcher.is_greater_than_or_equal_to(2)
    end
  end

  context 'with a model with a less than validation' do
    it 'match accordingly' do
      comparison_instance(:less_than => 2).should matcher.is_less_than(2)
    end
  end

  context 'with a model with a less than or equal to validation' do
    it 'match accordingly' do
      comparison_instance(:less_than_or_equal_to => 2).should matcher.is_less_than_or_equal_to(2)
    end
  end
